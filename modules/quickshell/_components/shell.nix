...
''
  ...
  `]
                  ...
                  onRead: function(line) {
                      if (line.startsWith("SSID:")) {
                          ...
                          if (!wifiEnabled) {
                              ...
                              network.icon = "󰈀"
                          } else if (network.type === "wifi" && network.connected) {
                              if (network.strength < 25) network.icon = "󰤟"
                              else if (network.strength < 50) network.icon = "󰤢"
                              else if (network.strength < 75) network.icon = "󰤥"
                              else network.icon = "󰤨"
              ...
              onExited: {
                  let changed = (network.connected !== _prevConnected || network.type !== _prevType)
                  _prevConnected = network.connected
                  _prevType = network.type
                  _stableCount = changed ? 0 : Math.min(_stableCount + 1, 5)
              }
          }
          Timer {
              interval: 3000
