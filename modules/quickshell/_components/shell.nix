...
''
  ...
  w=$(busctl get-property org.freedesktop.NetworkManager /org/freedesktop/NetworkManager org.freedesktop.NetworkManager.Wireless Enabled 2>/dev/null | grep -q true && echo 1 || echo 0)
            ...
            stdout: SplitParser {
                onRead: function(line) {
                    if (line.startsWith("SSID:")) {
                        ...
                        network.connected = state >= 50
                        let connType = parts[2]
                        if (connType.includes("wireless")) network.type = "wifi"
                        else if (connType.includes("ethernet")) network.type = "ethernet"
                        else if (network.connected) network.type = "other"
                        else network.type = "none"

                        if (!wifiEnabled || !network.connected || network.type === "none") {
                            network.icon = "󰤭"
